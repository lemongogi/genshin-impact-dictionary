...
import random
from infomultiply import info_weapon_level2
...
def charcater_info_place():
    ...
    character_ult_image.place(x=0, y=415)
    character_ult_text.place(x=90, y=430)
    character_weapon_level.place(x=243, y=210)
    character_weapon_multiply_frame.place(x=330, y=210)
    character_weapon_multiply.place(x=330, y=245)
...
def babara_info():
    ...
    if infostat==1:
        ...
        character_name.configure(text="바바라", fg="dodgerblue3", font=("굴림체",40), bg="light steel blue")
        character_eletype.configure(text="물속성", fg="dodgerblue3", font=("굴림체", 40), bg="light steel blue")
        character_weapon.configure(text="법구", fg="dodgerblue3", font=("굴림체", 40), bg="light steel blue")
        charcater_info_place()
        character_weapon_image.configure(image=orb)
        character_weapon_text.configure(text="주기가 4회인 물 원소 공격을 합니다\n강공격:짧은 영창 후 범위 물 원소피해를 가합니다\n낙하공격:착지 시 범위 내 적에게 물 원소 피해를 가합니다", fg="dodgerblue3", font=("굴림체", 18), bg="light steel blue")
        character_skill_image.configure(image=babara_skill_image)
        character_skill_text.configure(text="자신에게 노래의 고리를 부여하고 주위의 적에게 물 원소 피해를 가합니다.\n노래의 고리는 캐릭터를 전환해도 유지됩니다.\n노래의 고리를 가진 동안 바바라의 공격(강공격은 4배)이 명중 시 파티원 전원을 치유합니다\n노래의 고리가 있는 동안 주기적으로 현제 캐릭터를 치유하며 현제캐릭터와 주위의 적에게 습기를 부여합니다", fg="dodgerblue3", font=("굴림체", 12), bg="light steel blue")
        character_ult_image.configure(image=babara_ult_image)
        character_ult_text.configure(fg="dodgerblue3", font=("굴림체", 26), bg="light steel blue", text="주위 아군 및 자신의 파티원 전원이 회복됩니다")
...
def info_weapon_level(character_number):
    weapon_level=askinteger("일반 공격의 레벨", "일반공격의 레벨을 설정해주세요", minvalue=1, maxvalue=15)
    character_weapon_level.configure(text=weapon_level)
    info_weapon_level2(character_number, weapon_level, character_weapon_multiply, character_weapon_multiply_frame)
...
character_name=Label(window, text="바바라", fg="dodgerblue3", font=("굴림체",40), bg="light steel blue")
character_eletype=Label(window, text="물속성", fg="dodgerblue3", font=("굴림체", 40), bg="light steel blue")
character_weapon=Label(window, text="법구", fg="dodgerblue3", font=("굴림체", 40), bg="light steel blue")
...
character_ult_text=Label(window, fg="dodgerblue3", font=("굴림체", 26), bg="light steel blue", text="주위 아군 및 자신의 파티원 전원이 회복됩니다")
character_weapon_multiply_frame=Label(window, text="", bg="light steel blue", fg="dodgerblue3", font=("굴림체",18))
character_weapon_multiply=Label(window, text="", bg="light steel blue", fg="dodgerblue3", font=("굴림체",18))
character_weapon_level=Button(window, text="1", command=info_weapon_level_butten_ver, width=8, height=3)
